 ...
 *   the already-resolved YAML node value; may be async and may return any
 *   YAML-able value. `settings` is this transformer's own config, read from
...
async function _resolveDirective({directive, path, ctx}) {
  ...
  const at = `config transformer "!${name}" at "${location}"`;
  ...
  return key ? _memoize(ctx.cache, key, run) : run();
}
