...
export function _createSharedLoader({load}) {
  let promise;
  return () => {
    promise ??= load();
    return promise;
...
const readSharedAwsConfig = _createSharedLoader({
  load: () => readConfigFromAws().then(_parseAwsConfig)
});
...
async function _applyConfigFromAws({configType}) {
  const combined = await readSharedAwsConfig();
  const configYaml = combined[configType];
  if(configYaml === undefined) {
    return;
  }
  if(!_isObject(configYaml)) {
    throw safeError(
      `The AWS ${configType} config section must be an object`);
  }
  _assertSourcesAreNotReconfigured({configYaml, configType});
  const prepared = await _prepareConfig({configYaml, configType});
  // a transformer may return an object that expands into an arbitrary config
  // subtree, so the source guard must also run on the resolved config
  _assertSourcesAreNotReconfigured({configYaml: prepared, configType});
  _extend(true, config, prepared);
}
...
function _parseAwsConfig(content) {
  ...
  if(!_isObject(combined)) {
    throw safeError('The AWS bedrock config must be an object');
  }
  return combined;
}

function _assertSourcesAreNotReconfigured({configYaml, configType}) {
  const sourceConfig = configYaml?.['config-yaml'];
  if(sourceConfig !== undefined &&
    (!_isObject(sourceConfig) || Object.hasOwn(sourceConfig, 'sources'))) {
    throw safeError(
      'The AWS config source cannot reconfigure config sources in the ' +
      `${configType} config`);
  }
}
