...
function handleConfigLoadError(e, {configType}) {
  // only rethrow raw error in development to avoid leaking sensitive info
  ...
  throw new Error('Failed to load config: BEDROCK_CONFIG is invalid');
}
...
export function _applyConfigFromEnv({configType}) {
  ...
  try {
    ...
    handleConfigLoadError(e, {configType});
  }
}

// Exported for test purposes only
export function _applyConfig({configType}) {
