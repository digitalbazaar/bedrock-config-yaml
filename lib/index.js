...
export async function _applyConfig({configType}) {
  ...
  try {
    for(const type of types) {
      ...
      /*
      * We only want to load the config if it exists, and ignore otherwise.
      *
      * RACE CONDITION: technically the config could become un/accessible
      * between this check and the read. If the file is deleted or unreadable
      * after the check, the startup will fail. If it becomes readable after,
      * it will be ignored.
      *
      * We consider this acceptable edgecase behavior.
      */
      const configExists = await checkExists(configFile);
      if(configExists) {
        logger.debug(`"${type}" configuration found "${configFile}".`);
        const configRaw = await fs.readFile(configFile, 'utf8');
        let configYaml = jsYaml.load(configRaw);
