...
/* Spec:
  {
    // configuration sources that may provide the combined YAML config
    sources: {
      // AWS-backed configuration source
      aws: {
        // true to load the combined config from AWS
        enabled,
        // AWS runtime environment; currently only "nitro" is supported
        environment,
        // maximum time to wait for startup dependencies to become ready
        maxWaitMs,
        // EC2 instance tag whose value is a Secrets Manager SecretId
        secretIdTag
      }
    }
  }
