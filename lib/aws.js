...
const AWS_ENVIRONMENT_VARIABLE = 'BEDROCK_CONFIG_AWS_ENVIRONMENT';
...
export function getAwsSource() {
  ...
  // A deployment wrapper can select an AWS runtime environment before Bedrock
  // starts. This is bootstrap configuration: it must be available before the
  // external config it selects can be loaded. The marker is intentionally an
  // override so top-level applications can remain disabled and portable.
  const environmentOverride = process.env[AWS_ENVIRONMENT_VARIABLE];
  const enabled = environmentOverride !== undefined || source.enabled === true;
  if(!enabled) {
    return null;
  }

  const environment = environmentOverride ?? source.environment;
  ...
  if(!Number.isInteger(source.maxWaitMs) || source.maxWaitMs < 0) {
    throw new BedrockError(
